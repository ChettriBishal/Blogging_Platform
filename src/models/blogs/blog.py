"""This module contains various operations performed on a blogs"""
...
class Blog(Post):
    """
    Class containing various methods possible on a blogs
    """
    ...
    def set_blog_id(self, blog_id: str) -> None:
        """
        Method to set the blogs id to a blogs object
        """
        self.blog_id = blog_id

    def edit_content(self, new_content: str) -> Union[bool, None]:
        """
        Method to edit the blogs content associated to a blogs object
        """
        try:
            if self.blog_id is None:
                self.blog_id = Database.get_item(Sql.GET_BLOG_ID.value, (self.title, self.creator,))[0]

            Database.query_with_params(Sql.EDIT_BLOG_CONTENT.value, (new_content, self.blog_id,))
    ...
    def remove_content(self) -> Union[bool, None]:
        """
        Method to remove the content associated with a blogs object
        """
        try:
            if self.blog_id is None:
    ...
    def upvote(self, user_id: str) -> bool:
        """
        Method which allows user to upvote a blogs
        """
    ...
    def details(self) -> str:
        """
        Method to return the details of the blogs
        """
    ...
    def get_comments(self) -> Union[List[Comment], None]:
        """
        Method to get the comments present in the blogs
        """
    ...
    def remove_comments(self) -> None:
        """
        Method to remove all the comments present in the blogs
        """
    ...
    def remove_content_by_title(self) -> bool:
        """
        Method to remove the blogs content by the title name
        """
        if self.blog_id is None:
